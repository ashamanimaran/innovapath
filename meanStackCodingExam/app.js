var myApp=angular.module('myApp',['ngSanitize','ui.bootstrap','checklist-model']);
...
angular.module('myApp').controller('ModalInstanceCtrl', function ($scope, $uibModalInstance) {
  $scope.ok = function () {
      $scope.info.options=$scope.selectedOptions.options;
  ...
  $scope.upgradeAccount=function(){
      $scope.info.accType="standard";
      $scope.hideOptions=false; 
      $scope.options=["articles", "videos", "code camps", "paid projects", "cloud access"];
  }
  $scope.selectedOptions={
      options:[]
  }
  $scope.setAccTypeStandardPlus=function(){
      console.log($scope.selectedOptions.options);
      if($scope.selectedOptions.options.length>3){
          $scope.info.accType="plus"; 
      }
  }
  $scope.setAccTypeLight=function(){
      $scope.info.accType="light";
  }
